/*
 * https://github.com/m5stack/M5-ProductExampleCodes/blob/master/Hat/JoyC/Arduino/JoyC/JoyC.ino
 */
...
void setup() {
  ...
  */
  ...
  Disbuff.fillRect(0,20,135,140,BLACK);
...
uint16_t AngleBuff[4]; // Stick LLR, LUD, RLR, RUD
uint32_t count = 0;
void loop() {
  ...
  {
    SendBuff[3] = map(AngleBuff[0],0,4000,0,200); // Left L/R
    SendBuff[4] = map(AngleBuff[1],0,4000,0,200); // Left U/D
    SendBuff[5] = map(AngleBuff[2],0,4000,0,200); // Right L/R
    SendBuff[6] = map(AngleBuff[3],0,4000,0,200); // Right U/D
    /*
    Disbuff.pushImage(0,0,20,20,(uint16_t *)connect_on);
    Disbuff.pushSprite(0,0);
    count = 0;
    */
    ...
    else
    {
      // Stop
      //SendBuff[6] = 0x00;
      if( ( SendBuff[6] < 180  )&&( SendBuff[6] > 20 ))
      {
        // When the right stick (U/D) is almost upright
        SendBuff[6] = 0x64; // = 100
      }
    }
    SendUDP();
  ...
  Disbuff.fillRect( 0,30,135,130,BLACK);
