...
void Servo_angle(uint8_t Servo_ch, uint8_t deg)
{
  deg = min(90, int(deg));
  deg = max(0, int(deg));
  I2CWrite1Byte((Servo_ch - 1) | 0x10 ,int(deg));
}
...
void Servo_pulse(uint8_t Servo_ch, uint16_t width)
{
  ...
  I2CWrite1Byte((Servo_ch - 1) | 0x20, width);
}
    ...
    if(( udodata[0] == 0xAA )&&( udodata[1] == 0x55 )&&( udodata[7] == 0xee ))
    {
      ...
      else
      {
        ...
        { 
          ...
          if ( ( udodata[6] <  20 ) && degree >  0 + 2 ) degree = degree - 2;
          if ( ( udodata[6] > 180 ) && degree < 90 - 2 ) degree = degree + 2;
